...
const CARGO_TOML: &'static str = include_str!("../../template/Cargo.toml");
const RUSTFMT_TOML: &'static str = include_str!("../../template/rustfmt.toml");
const LIB_RS: &'static str = include_str!("../../template/src/lib.rs");
const REQUEST_RS: &'static str = include_str!("../../template/src/request.rs");
const ROUTE_RS: &'static str = include_str!("../../template/src/route.rs");
const CLIENT_RS: &'static str = include_str!("../../template/src/client.rs");
const ERROR_RS: &'static str = include_str!("../../template/src/error.rs");
...
impl<'a> CrateWriter<'a> {
    ...
    pub fn write(&self) -> std::io::Result<()> {
        ...
        std::fs::write(
            std::path::Path::new(path).join("Cargo.toml"),
            CARGO_TOML,
        )?;
        std::fs::write(
        ...
        std::fs::write(
            std::path::Path::new(path).join("src/lib.rs"),
            LIB_RS,
        )?;
        std::fs::write(
        ...
        std::fs::write(
            std::path::Path::new(path).join("src/route.rs"),
            ROUTE_RS,
        )?;
        std::fs::write(
            std::path::Path::new(path).join("src/client.rs"),
            CLIENT_RS,
        )?;
        std::fs::write(
            std::path::Path::new(path).join("src/error.rs"),
            ERROR_RS,
        )?;
        std::fs::write(
        ...
        let mut methods_mod_file = std::fs::File::create(
            methods_mod_path.join("mod.rs"),
        )?;
